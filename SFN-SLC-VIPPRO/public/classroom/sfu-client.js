export class SkySfuClient {
  ...
  async init() {
    ...
    const r = await this.api('/api/live/sfu/session/new', {
      method: 'POST',
  ...
  async publishTrack(track, source='camera') {
    ...
    return this._serial(async () => {
      ...
      const result = await this.api(`/api/live/sfu/session/${encodeURIComponent(this.sessionId)}/tracks`, {
        method: 'POST',
        body: JSON.stringify({
  ...
  async unpublish(source) {
    ...
    try {
      await this.api(`/api/live/sfu/session/${encodeURIComponent(this.sessionId)}/unpublish`, {
        method: 'POST',
    ...
    try {
      ...
      await this._serial(async () => {
        ...
        if (result.requiresImmediateRenegotiation && result.sessionDescription) {
          ...
          await this.api(`/api/live/sfu/session/${encodeURIComponent(this.sessionId)}/renegotiate`, {
            method: 'PUT',
            body: JSON.stringify({
              class_id: this.classId,
              access_token: this.accessToken,
  ...
  async recoverIce() {
    ...
    try {
      ...
      await this.pc.setLocalDescription(offer);
      const result = await this.api(`/api/live/sfu/session/${encodeURIComponent(this.sessionId)}/renegotiate`, {
        method: 'PUT',
        body: JSON.stringify({
          class_id: this.classId,
          access_token: this.accessToken,
  ...
  async close() {
    ...
    if (this.sessionId) {
      try {
        await this.api(`/api/live/sfu/session/${encodeURIComponent(this.sessionId)}/end`, {
