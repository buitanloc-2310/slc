...
const EXTRA_V10_SCHEMA_SQL = `PRAGMA foreign_keys = ON;
...
async function installSchema(env){
  await env.DB.exec(AUTO_SCHEMA_SQL);
  await env.DB.exec(EXTRA_V9_SCHEMA_SQL);
  await env.DB.exec(EXTRA_V10_SCHEMA_SQL);
  return true;
}
